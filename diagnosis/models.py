...
class Bill(models.Model):
    bill_number = models.CharField(max_length=20, unique=True, editable=False)  # autogenerated
    ...
    total_amount = models.IntegerField(editable=False)  # set from test_type.price, no manual input
    ...
    def clean(self):
        ...
        if total != paid + center_disc + doctor_disc or total < paid + center_disc + doctor_disc:
            raise ValidationError(
                f"Total amount ({total}) must be equal to paid amount ({paid}) + center discount ({center_disc}) + doctor discount ({doctor_disc})."
            )
    ...
    # Generate bill number if not already set
        ...
        # Overwrite total_amount from test_type
        if self.test_type:
        ...
        if self.referred_by_doctor and self.test_type:
            doctor = self.referred_by_doctor
            category = self.test_type.category.lower()
            ...
            elif doctor_disc == 0 and center_disc > 0:
                doctor_incentive = full_incentive
        ...
        # Final incentive stored as integer
        self.incentive_amount = doctor_incentive
        self.clean() # Call clean method to validate the data
